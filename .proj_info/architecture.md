...
Основные UI-компоненты используют glassmorphism и динамическую типографику:
- **Масштабирование заголовков**: Основной заголовок (`.main-title`) имеет размер 42px. Для русской локализации используется адаптивный размер **32px** для предотвращения разрыва строк и сохранения баланса.
- **Glass Pill Nav** (`.glass-pill-nav`): полупрозрачные rounded-pill контейнеры с `backdrop-filter: blur(16px)`.
- **Pill Slider** (`.pill-slider`): скользящий индикатор, управляемый через `updatePillSlider()`.
- **Glass Dropdown** (`.glass-dropdown`): выпадающий список для выбора темы оформления.
- **Glass Toggle** (`.glass-toggle`): тумблер с стеклянной дорожкой и круглым knob.
- **Glass Cards** (`.settings-glass-card`): карточки с `backdrop-filter: blur(20px)`, `border-radius: 16px`.
